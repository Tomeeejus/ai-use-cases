...
const UseCaseGrid = () => {
  ...
  const filteredAndSortedUseCases = useMemo(() => {
    let filtered = useCases.filter(useCase => {
      ...
      const matchesSearch = searchQuery === '' || useCase.title.toLowerCase().includes(searchQuery.toLowerCase()) || useCase.description.toLowerCase().includes(searchQuery.toLowerCase()) || useCase.category.toLowerCase().includes(searchQuery.toLowerCase());
        ...
        <div className="space-y-8">
          ...
          <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row gap-4 items-center">
              <div className="relative flex-1 max-w-md">
                ...
                <Input placeholder="Search AI solutions..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-10 bg-card/50 backdrop-blur-sm border-border/50 focus:border-primary/50 transition-colors" />
              </div>
              
              <div className="flex items-center gap-3">
                <select value={sortBy} onChange={e => setSortBy(e.target.value)} className="bg-card/50 backdrop-blur-sm border border-border/50 rounded-md px-3 py-2 text-sm text-foreground focus:border-primary/50 outline-none transition-colors">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-border/50 pb-4">
            <div className="flex items-center gap-4">
              ...
              {searchQuery && <span className="text-xs bg-primary/10 text-primary px-2 py-1 rounded-full">
          ...
          {/* Grid/List View */}
          <div className={`${viewMode === 'grid' ? 'grid md:grid-cols-2 lg:grid-cols-3 gap-6' : 'space-y-4'} animate-fade-in`}>
            {filteredAndSortedUseCases.map((useCase, index) => <div key={index} className="animate-scale-in" style={{
          ...
          {filteredAndSortedUseCases.length === 0 && <div className="text-center py-12 animate-fade-in">
          ...
          {/* Load More */}
          {filteredAndSortedUseCases.length > 0 && <div className="text-center pt-8 animate-fade-in">
